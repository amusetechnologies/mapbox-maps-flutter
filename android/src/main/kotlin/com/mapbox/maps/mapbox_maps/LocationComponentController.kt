...
class LocationComponentController(
  ...
  private fun setExternalLocation(call: MethodCall, result: MethodChannel.Result) {
    ...
    // iOS's Location.floor. amuse_map's FloorSwitcherController handles floor
    // entirely separately via notifyFloorChange.
   ...
   * normal GPS," matching how `amuse_map`'s `clearExternalUserLocation()` is
   * used today (on a location-stream error, where presenting a stale
   * synthetic position would be worse than falling back to GPS).
   */

...
final class LocationController: _LocationComponentSettingsInterface {
    ...
    private func setExternalLocation(arguments: Any?, result: @escaping FlutterResult) {
        ...
        let location = Location(
            ...
            source: "amuse-external-override",
            extra: nil)
    ...
    /// "go back to normal GPS," matching how `amuse_map`'s
    /// `clearExternalUserLocation()` is used today (on a location-stream
    /// error, where presenting a stale synthetic position would be worse
    /// than falling back to GPS).
